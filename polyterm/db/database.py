...
class Database:
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS arb_executions (
                    ...
                    yes_fill_price  REAL DEFAULT 0.0,
                    no_fill_price   REAL DEFAULT 0.0,
                    yes_order_id    TEXT DEFAULT "",
                    no_order_id     TEXT DEFAULT "",
                    total_invested  REAL DEFAULT 0.0,
                    spread          REAL DEFAULT 0.0,
                    net_edge        REAL DEFAULT 0.0,
                    kelly_fraction  REAL DEFAULT 0.0,
                    expected_profit REAL DEFAULT 0.0,
                    both_success    INTEGER DEFAULT 0,
                    yes_error       TEXT DEFAULT "",
                    no_error        TEXT DEFAULT "",
                    timestamp       TIMESTAMP NOT NULL
                )
            """)

            # Arb decisions table (execute/skip + motivos + métricas)
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS arb_decisions (
                    ...
                    market_id       TEXT NOT NULL,
                    market_title    TEXT DEFAULT "",
                    decision        TEXT NOT NULL,         -- EXECUTE | SKIP | ERROR
                    reason          TEXT DEFAULT "",        -- edge_below_threshold | whale_filter | exec_exception | ...
                    data            TEXT DEFAULT "{}",      -- JSON: métricas/inputs (edge, slip, kelly, etc.)
                    timestamp       TIMESTAMP NOT NULL
                )
            """)
    ...
    # ── Mejoras: Arb execution journal ──────────────────────────────────────

    def save_arb_execution(self, execution) -> int:
        """Persiste una ArbExecution en arb_executions."""
        from datetime import datetime
        with self._get_connection() as conn:
            cursor = conn.execute(
                "INSERT INTO arb_executions "
                "(opportunity_id,market_id,market_title,mode,"
                "yes_token_id,no_token_id,yes_fill_price,no_fill_price,"
                "yes_order_id,no_order_id,total_invested,spread,net_edge,"
                "kelly_fraction,expected_profit,both_success,"
                "yes_error,no_error,timestamp) "
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    execution.opportunity_id, execution.market_id,
                    execution.market_title, execution.yes_trade.mode,
                    execution.yes_trade.token_id, execution.no_trade.token_id,
                    execution.yes_trade.fill_price, execution.no_trade.fill_price,
                    execution.yes_trade.order_id or "",
                    execution.no_trade.order_id or "",
                    execution.total_invested, execution.spread,
                    execution.net_edge, execution.kelly_fraction,
                    execution.expected_net_profit,
                    int(execution.both_success),
                    execution.yes_trade.error or "",
                    execution.no_trade.error or "",
                    datetime.fromtimestamp(execution.timestamp),
                )
            )
            return cursor.lastrowid

    def get_execution_summary(self, hours: float = 24) -> dict:
        """Resumen de performance de ejecuciones."""
        from datetime import datetime
        cutoff = datetime.now().timestamp() - hours * 3600
        with self._get_connection() as conn:
            row = conn.execute(
                "SELECT COUNT(*) as total, SUM(both_success) as successful,"
                "SUM(total_invested) as total_invested, AVG(net_edge) as avg_edge,"
                "MAX(net_edge) as max_edge, SUM(expected_profit) as expected_pnl"
                " FROM arb_executions WHERE timestamp >= datetime(?,'unixepoch')",
                (cutoff,)
            ).fetchone()
            return {
                "hours": hours, "total_arbs": row["total"] or 0,
                "successful": row["successful"] or 0,
                "total_invested": row["total_invested"] or 0.0,
                "avg_edge": row["avg_edge"] or 0.0,
                "max_edge": row["max_edge"] or 0.0,
    ...
    def get_recent_executions(self, limit: int = 50) -> list:
        ...
        with self._get_connection() as conn:
            rows = conn.execute(
                "SELECT * FROM arb_executions ORDER BY timestamp DESC LIMIT ?",
                (limit,)
            ).fetchall()
    ...
    def save_arb_decision(
        ...
        """Persiste una decisión del pipeline (execute/skip/error) para auditoría."""
        if data is None:
            data = {}
        ...
        with self._get_connection() as conn:
            cur = conn.execute(
                ...
                "(opportunity_id, market_id, market_title, decision, reason, data, timestamp) "
                "VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
                    opportunity_id,
                    market_id,
    ...
    def get_decision_summary(self, hours: float = 24) -> dict:
        """Resumen de decisiones por tipo/motivo para diagnosticar ausencia de trades."""
        cutoff = datetime.now().timestamp() - hours * 3600
        with self._get_connection() as conn:
            row = conn.execute(
                "SELECT COUNT(*) as total,"
                "SUM(CASE WHEN decision='EXECUTE' THEN 1 ELSE 0 END) as execute_cnt,"
                "SUM(CASE WHEN decision='SKIP' THEN 1 ELSE 0 END) as skip_cnt,"
                "SUM(CASE WHEN decision='ERROR' THEN 1 ELSE 0 END) as err_cnt "
                "FROM arb_decisions WHERE timestamp >= datetime(?,'unixepoch')",
                (cutoff,),
            ).fetchone()
            ...
            return {
                ...
                "execute": row["execute_cnt"] or 0,
                "skip": row["skip_cnt"] or 0,
                "error": row["err_cnt"] or 0,
                "by_reason": [{"reason": r["reason"] or "", "count": r["c"] or 0} for r in by_reason],
            }
